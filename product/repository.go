...
func (repo *repository) InsertProduct(params *getAddProductRequest) (int64, error) {
	...
	result, err := repo.db.Exec(sql, params.ProductCode, params.ProductName, params.Category,
		params.Description, params.ListPrice, params.StandardCost)
	helper.Catch(err)
...
func (repo *repository) GetBestSellers() ([]*ProductTop, error) {
	const sql = `select 
					od.product_id,
					p.product_name,
					sum(od.quantity * od.unit_price) as vendido
...
func (repo *repository) GetTotalVentas() (float64, error) {
	const sql = `SELECT SUM(od.quantity * od.unit_price) AS vendido FROM order_details od `
	var total float64
	row := repo.db.QueryRow(sql)
	err := row.Scan(&total)
	helper.Catch(err)
